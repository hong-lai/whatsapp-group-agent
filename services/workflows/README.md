...
Keep `AVAILABLE_WORKFLOW_NAMES` in `packages/shared/src/config.ts` aligned with `_AVAILABLE` in `agent_workflows/registry.py`.
...
`ingest` enqueues BullMQ jobs when `WORKFLOWS_ENABLED=true` (default in compose).  
The dashboard API can also re-enqueue from the admin UI.  
...
QR login is on the ingest service:
...
3. Mirror the name in `packages/shared/src/config.ts` `AVAILABLE_WORKFLOW_NAMES`  
4. Set `ENABLED_WORKFLOWS=daily_site_report,<name>`  
5. Rebuild: `docker compose up -d --build workflows`
...
The API service fans that out on SSE `GET /api/events/report-processed` so the web dashboard can show a toast and refresh the reports list.
