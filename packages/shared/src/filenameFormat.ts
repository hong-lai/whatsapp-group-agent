...
const FILENAME_FORMAT_CACHE_KEY = 'settings:filename-format'

async function getFilenameFormatCache<T>(): Promise<T | undefined> {
    const raw = await redis.get(FILENAME_FORMAT_CACHE_KEY)
...
async function setFilenameFormatCache(value: unknown): Promise<void> {
    await redis.set(FILENAME_FORMAT_CACHE_KEY, JSON.stringify(value))
}
import {
    fileStem,
    firstAvailableName,
